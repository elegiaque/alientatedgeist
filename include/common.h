#ifndef STARDUST_COMMON_H
#define STARDUST_COMMON_H

//
// system related headers
#include <windows.h>
#include <type_traits>
#include <concepts>
#include <winhttp.h>
#include <winsock2.h>
#include <ws2tcpip.h>

//
// stardust related headers
#include <constexpr.h>
#include <macros.h>
#include <memory.h>
#include <native.h>
#include <resolve.h>

extern "C" auto RipData() -> uintptr_t;
extern "C" auto RipStart() -> uintptr_t;

#if defined( DEBUG )
#define DBG_PRINTF( format, ... ) { ntdll.DbgPrint( symbol<PCH>( "[DEBUG::%s::%d] " format ), symbol<PCH>( __FUNCTION__ ), __LINE__, ##__VA_ARGS__ ); }
#else
#define DBG_PRINTF( format, ... ) { ; }
#endif

#ifdef _M_X64
#define END_OFFSET 0x10
#else
#define END_OFFSET 0x10
#endif


typedef struct {
    SOCKET sock;
} CH_ENTRY;


namespace stardust
{
    template <typename T>
    inline T symbol(T s) {
        return reinterpret_cast<T>(RipData()) - (reinterpret_cast<uintptr_t>(&RipData) - reinterpret_cast<uintptr_t>(s));
    }

    class instance {
        struct {
            uintptr_t address;
            uintptr_t length;
        } base = {};

        HINTERNET hSocket = NULL;

        private:
            CH_ENTRY* channels;


        struct {
            uintptr_t handle;

            struct {
                D_API( LoadLibraryA )
                D_API( GetProcAddress )
                D_API( CreateThread )
                D_API( CloseHandle )
                D_API( VirtualAlloc )
                D_API( VirtualFree )
            };
        } kernel32 = {
            RESOLVE_TYPE( LoadLibraryA ),
            RESOLVE_TYPE( GetProcAddress ),
            RESOLVE_TYPE( CreateThread ),
            RESOLVE_TYPE( CloseHandle ),
            RESOLVE_TYPE( VirtualAlloc ),
            RESOLVE_TYPE( VirtualFree )
        };
        
        struct {
            uintptr_t handle;

            struct
            {
                D_API( WinHttpOpen )
                D_API( WinHttpConnect )
                D_API( WinHttpOpenRequest )
                D_API( WinHttpSetOption )
                D_API( WinHttpSendRequest )
                D_API( WinHttpReceiveResponse )
                D_API( WinHttpWebSocketCompleteUpgrade )
                D_API( WinHttpWebSocketReceive )
                D_API( WinHttpWebSocketSend )
                D_API( WinHttpCloseHandle )
            };
        } winhttp = {
            RESOLVE_TYPE( WinHttpOpen ),
            RESOLVE_TYPE( WinHttpConnect ),
            RESOLVE_TYPE( WinHttpOpenRequest ),
            RESOLVE_TYPE( WinHttpSetOption ),
            RESOLVE_TYPE( WinHttpSendRequest ),
            RESOLVE_TYPE( WinHttpReceiveResponse ),
            RESOLVE_TYPE( WinHttpWebSocketCompleteUpgrade ),
            RESOLVE_TYPE( WinHttpWebSocketReceive ),
            RESOLVE_TYPE( WinHttpWebSocketSend ),
            RESOLVE_TYPE( WinHttpCloseHandle )
        };

        struct {
            uintptr_t handle;

            struct {
                D_API( WSAStartup )
                D_API( WSACleanup )
                D_API( getaddrinfo )
                D_API( freeaddrinfo )
                D_API( socket )
                D_API( connect )
                D_API( closesocket )
                D_API( send )
                D_API( recv )
                D_API( shutdown )
            };
        } ws2_32 = {
            RESOLVE_TYPE( WSAStartup ),
            RESOLVE_TYPE( WSACleanup ),
            RESOLVE_TYPE( getaddrinfo ),
            RESOLVE_TYPE( freeaddrinfo ),
            RESOLVE_TYPE( socket ),
            RESOLVE_TYPE( connect ),
            RESOLVE_TYPE( closesocket ),
            RESOLVE_TYPE( send ),
            RESOLVE_TYPE( recv ),
            RESOLVE_TYPE( shutdown )
        };

        struct {
            uintptr_t handle;

            struct {
#ifdef DEBUG
                D_API( DbgPrint )
#endif
            };
        } ntdll = {
#ifdef DEBUG
            RESOLVE_TYPE( DbgPrint )
#endif
        };

    public:
        explicit instance();

        auto start(
            _In_ void* arg
        ) -> void;

        auto initChannels(
            void
        ) -> void;

        static DWORD WINAPI forwardThread(
            LPVOID arg
        );

        auto wsSend(
            const unsigned char *buf,
            DWORD buflen
        ) -> int;

        auto sendFrame(
            unsigned char cmd,
            unsigned short channel,
            const unsigned char *data,
            unsigned short pLength
        ) -> int;

        auto mapAdd(
            uint16_t channel,
            SOCKET s
        ) -> int;

        auto mapFind(
            uint16_t channel
        ) -> SOCKET;

        auto mapRemove(
            uint16_t channel
        ) -> void;

        auto hOpen(
            unsigned short channel,
            const unsigned char *data,
            unsigned short pLength
        ) -> void;

        auto hData(
            unsigned short channel,
            const unsigned char *data,
            unsigned short pLength
        ) -> void;

        auto hClose(
            unsigned short channel
        ) -> void;

        auto wsConnect(
            _In_ const wchar_t *host,
            _In_ const wchar_t *path
        ) -> HINTERNET;

        auto wsLoop(
        ) -> void;
    };

    template<typename T = char>
    inline auto declfn hash_string(
        _In_ const T* string
    ) -> uint32_t {
        uint32_t hash = 0x811c9dc5;
        uint8_t  byte = 0;

        while ( * string ) {
            byte = static_cast<uint8_t>( * string++ );

            if ( byte >= 'a' ) {
                byte -= 0x20;
            }

            hash ^= byte;
            hash *= 0x01000193;
        }

        return hash;
    }
}


#endif //STARDUST_COMMON_H
